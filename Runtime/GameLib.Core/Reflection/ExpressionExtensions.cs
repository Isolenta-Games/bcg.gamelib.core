	...
	public static class ExpressionExtensions
	{
		...
		public static Action<T, TProperty> ToSetter<T, TProperty>(this Expression<Func<T, TProperty>> expression)
		{
			...
			else if (memberExpression.Member is FieldInfo field)
			{
				...
				var newExpression =
					Expression.Lambda<Action<T, TProperty>>(
						Expression.Assign(fieldExp, parameterTProperty),
						parameterT,
						parameterTProperty
					);
		...
		/// </summary>
		public static Func<T, TProperty> ToGetter<T, TProperty>(this Expression<Func<T, TProperty>> expression)
		{
		...
		public static TValue InvokeGetter<T, TValue>(this Expression<Func<T, TValue>> expression, T x) where T : class
		{
			...
			// The first one is the getter we treat as a setter
			{
				...
				if (pi != null)
				{
					return (TValue)pi.GetValue(targetObject);
				}
				else
				{
					var fi = (FieldInfo)members[0];
