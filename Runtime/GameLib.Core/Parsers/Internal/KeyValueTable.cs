...
namespace GameLib.Core.Parsers.Internal
{
	...
	{
		private readonly Dictionary<string, IRawTableRow> _values = new Dictionary<string, IRawTableRow>(32, StringComparer.InvariantCultureIgnoreCase);
		private readonly HashSet<string> _headers = new HashSet<string>(StringComparer.InvariantCultureIgnoreCase);
		...
		{
			if (!table.HasColumnInHeader(keyColumn))
			{
				throw new RowValueException($"Required column in header is not found: {keyColumn}");
			}

			if (!table.HasColumnInHeader(valueColumn))
			{
				throw new RowValueException($"Required column in header is not found: {valueColumn}");
			}
		...
		private void Initialize(IEnumerable<IRawTableRow> rows, IEnumerable<string> headers, string keyColumn, string valueColumn)
		{
			...
			foreach (var row in rows)
			{
				...
				{
					_firstRow = row;
					if (!_firstRow.HasColumnInHeader(keyColumn))
					{
						throw new RowValueException($"Required column in header is not found: {keyColumn}");
					}

					if (!_firstRow.HasColumnInHeader(valueColumn))
					{
						throw new RowValueException($"Required column in header is not found: {valueColumn}");
					}
				}
				...
				{
					throw row.MakeError($"Duplicate key found in column '{keyColumn}': '{key}'");
				}
		...
		private IRawTableRow GetRowByKey(string key)
		{
			return !key.IsNullOrEmpty() ? _values.FirstOrDefault(key) : null;
		}
		...
		public bool IsEmpty(string columnName)
		{
			...
			{
				return _firstRow?.IsEmpty(columnName) ?? true;
			}
		...
		public string RawValue(string columnName)
		{
			if (_headers.Contains(columnName))
			{
				return _firstRow?.RawValue(columnName);
			}
