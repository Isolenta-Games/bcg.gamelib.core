...
{
	public static class RawTableRowExtensions
	{
		/// <summary>
		/// interpret table as key=value pairs with two headers and use this as raw table row
		/// keys must be unique!
		/// </summary>
		public static IRawTableRow ToKeyValue(this IEnumerable<IRawTableRow> rows, string keyColumn, string valueColumn)
		{
			return new KeyValueTable(rows, keyColumn, valueColumn);
		}
		...
		/// keys must be unique!
		/// </summary>
		public static IRawTableRow ToKeyValue(this RawTable table, string keyColumn, string valueColumn)
		{
			return new KeyValueTable(table, keyColumn, valueColumn);
		}
		...
		/// </summary>
		public static IRawTableRow Join(IEnumerable<IRawTableRow> rows)
		{
		...
		/// headers must be unique in all rows
		/// </summary>
		public static IRawTableRow Join(this IRawTableRow row, IRawTableRow otherRow, params IRawTableRow[] extraRows)
		{
			return new JoinedRow(extraRows.Append(row).Append(otherRow));
		}
		
		/// <summary>
		/// filter columns by name prefix and optionally remove prefix from column name
		/// </summary>
		public static IRawTableRow SelectColumns(this IRawTableRow row, string prefix, bool removePrefix)
		{
			return new FilteredRow(row, prefix, removePrefix);
		}
		...
		{
			return RawReplace(row, new Dictionary<string, string>() {{columnName, newRawValue}});
		}

		/// <summary>
		/// replace one RAW value with another
		/// </summary>
		private static IRawTableRow RawReplace(IRawTableRow row, Dictionary<string, string> columnValues)
		{
			return new ReplaceRow(row, columnValues);
		}

		public static string FormatLocation(string context, object rowIndex)
		{
			return $"{context}[row={rowIndex}]";
		}

		public static IRawTableRow MakeRow(Dictionary<string, string> keys2Values, string contextName = "",
			int rowIndex = 0)
		{
			...
			return new RawTableRow(rowIndex, RawTableRowExtensions.FormatLocation(contextName, rowIndex),
				kvp.ToDictionary(x => x.Key, x => x.Index, StringComparer.InvariantCultureIgnoreCase),
		...
		public static string GetString(this IRawTableRow row, string columnName)
		{
			...
			if (raw == null)
			{
				throw row.MakeError($"'{columnName}' column not found or empty!'");
			}
		...
		{
			if (!row.RawValue(columnName, out var raw))
			{
				return defaultVal ?? throw row.MakeError($"'{columnName}' column not found or empty!'");
			}

			var v = raw?.Trim().ToLowerInvariant();

			if (v == "true" || v == "1" || v == "yes" || v == "y" || v == "+" || v == "x")
			{
				return true;
			}

			if (v.IsNullOrEmpty() || v == "false" || v == "0" || v == "no" || v == "n" || v == "-")
			{
				return false;
			}
		...
		public static DateTime GetDateTimeUTC(this IRawTableRow row, string columnName, string format, DateTime? defaultVal = null)
		{
			...
			{
				return defaultVal ?? throw row.MakeError($"Cannot convert column '{columnName}' ({raw}) to DateTime!");
			}
		...
		public static Range GetRange(this IRawTableRow row, string columnName)
		{
			...
			if (items.IsNullOrEmpty() || items.Length != 1 && items.Length != 2)
			{
				throw row.MakeError($"Must be 'int-int' or 'int' in '{columnName}'");
			}
		...
		public static RangeF GetRangeF(this IRawTableRow row, string columnName)
		{
			...
			if (items.IsNullOrEmpty() || items.Length != 1 && items.Length != 2)
			{
				throw row.MakeError($"Must be 'float-float' or 'float' in '{columnName}'");
			}

			return items.Length == 1 ? new RangeF(items[0], items[0]) : new RangeF(items[0], items[1]);
		}

		public static Range GetRange(this IRawTableRow row, string columnWithMin, string columnWithMax)
		{
			return new Range(row.GetInt(columnWithMin), row.GetInt(columnWithMax));
		}

		public static RangeF GetRangeF(this IRawTableRow row, string columnWithMin, string columnWithMax)
		{
		...
		{
			if (valueType == TypeOf<string>.Raw)
			{
				return row.GetString(columnName);
			}

			if (valueType == TypeOf<int>.Raw)
			{
				return row.GetInt(columnName);
			}

			if (valueType == TypeOf<float>.Raw)
			{
				return row.GetFloat(columnName);
			}

			if (valueType == TypeOf<double>.Raw)
			{
				return row.GetDouble(columnName);
			}

			if (valueType == TypeOf<bool>.Raw)
			{
				return row.GetBool(columnName);
			}

			if (valueType == TypeOf<BigInteger>.Raw)
			{
				return row.GetBigInteger(columnName);
			}

			if (valueType == TypeOf<Range>.Raw)
			{
				return row.GetRange(columnName);
			}

			if (valueType == TypeOf<RangeF>.Raw)
			{
				return row.GetRangeF(columnName);
			}
