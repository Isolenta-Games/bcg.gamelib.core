...
public static partial class EnumerableExtensions
{
		...
		if (source != null)
		{
			foreach (var item in source)
			{
				...
				if (!result.IsValidIndex(index))
					throw new IndexOutOfRangeException($"{index} is not valid for array [0..{result.Length - 1}]");
		...
		where ResultT : class
	{
		if (source == null)
			return new ResultT[0];
		return source.Select(accessor).Where(x => x != null);
	}
	...
	/// take last N items from collection preserving theirs order
	/// </summary>
	public static IEnumerable<T> TakeLast<T>(this T[] source, int n)
	{
		return source.Skip(Math.Max(0, source.Length - n));
	}
	...
	public static ValueT FirstOrDefault<KeyT, ValueT>(this Dictionary<KeyT, ValueT> obj, KeyT key)
	{
		return obj != null ? (obj.TryGetValue(key, out var result) ? result : default(ValueT)) : default(ValueT);
	}

	/// <summary>
	/// return first or default item of specific type
	/// </summary>
	public static ResultT FirstOrDefaultType<ResultT>(this IEnumerable obj)
	{
		return obj.OfType<ResultT>().FirstOrDefault();
	}

	/// <summary>
	/// return first or default item of specific type and condition
	/// </summary>
	public static ResultT FirstOrDefaultType<ResultT>(this IEnumerable obj, Func<ResultT, bool> predicate)
	{
		return obj.OfType<ResultT>().FirstOrDefault(predicate);
	}
